...
if st.session_state.get("analyzed"):
    ...
    with tab_map["Keywords"]:
        ...
        with c1:
            ...
            if result["matched"]:
                pills = "".join(f'<span class="pill pill-found">{kw}</span>' for kw in result["matched"])
        ...
        with c2:
            ...
            if result["missing"]:
                pills = "".join(f'<span class="pill pill-missing">{kw}</span>' for kw in result["missing"])
                st.markdown(pills, unsafe_allow_html=True)
